...
MICplus <- c("forgings","foundries","steel")
...
```
```{r}
# Compute average hourly wage for censusMIC + MICplus
avg_censusMIC_MICplus <- df %>%
...
# Compute average hourly wage for nonMIC
avg_nonMIC <- df %>%
  filter(industry %in% nonMIC) %>%
  group_by(year) %>%
  summarize(avg_prod_hr_earn = mean(prod_hr_earn, na.rm = TRUE)) %>%
...
# Extract manufacturing hourly wage
manufacturing_df <- df %>%
...
plot_df <- bind_rows(avg_censusMIC_MICplus, avg_nonMIC, manufacturing_df)

# Plot
ggplot(plot_df, aes(x = year, y = avg_prod_hr_earn, color = group)) +
  geom_line(size = 1.5) +
  geom_point(size = 2) +
  scale_x_continuous(breaks = seq(min(plot_df$year), max(plot_df$year), by = 2)) +
  labs(
    title = "Average Hourly Wage: MIC vs Manufacturing Avg vs Consumer",
    x = "Year",
    y = "Hourly Wage (Real $)",
    color = "Group"
  ) +
