...
censusMIC <- c("aircraft", "ordnance","communication", "ship", "electronics")
...
```
```{r}
# Filter for censusMIC industries and manufacturing
censusMIC_industries <- df %>%
  filter(industry %in% censusMIC | industry == "manufacturing")

# Create ratio to manufacturing for each industry
censusMIC_ratio_df <- censusMIC_industries %>%
  group_by(year) %>%
  mutate(manuf_wage = prod_hr_earn[industry == "manufacturing"]) %>%
  ungroup() %>%
  mutate(ratio_to_manuf = prod_hr_earn / manuf_wage) %>%
  filter(industry != "manufacturing")  # Remove manufacturing itself

# Plot each censusMIC industry against manufacturing baseline
ggplot(censusMIC_ratio_df, aes(x = year, y = ratio_to_manuf, color = industry)) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray50", size = 1) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  scale_x_continuous(breaks = seq(min(censusMIC_ratio_df$year), 
                                 max(censusMIC_ratio_df$year), by = 2)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Hourly Wage Relative to Manufacturing by Census MIC Industry",
    subtitle = "1.0 = Equal to Manufacturing Wage",
    x = "Year",
    y = "Wage Relative to Manufacturing",
    color = "Census MIC Industry"
  ) +
  theme_minimal() +
  theme(
...
reg_df <- df %>%
  filter(type %in% c("censusMIC", "MICplus", "nonMIC", "manufacturing")) %>%
  group_by(year, type) %>%
  summarize(avg_wage = mean(prod_hr_earn, na.rm = TRUE), .groups = "drop")

reg_df$type <- relevel(as.factor(reg_df$type), ref = "manufacturing")
wage_lm <- lm(avg_wage ~ year + type, data = reg_df)
summary(wage_lm)
